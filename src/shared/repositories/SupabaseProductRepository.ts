  ...
  async searchProduct(identifier: string): Promise<Product | null> {
    ...
    if (this.pgPool) {
      try {
        ...
        if (res.rows && res.rows.length > 0) return this.rowToProduct(res.rows[0]);
      } catch (err) {
  ...
  async getAllProducts(): Promise<Product[]> {
    if (this.pgPool) {
      try {
        ...
        if (res.rows) return res.rows.map((r: any) => this.rowToProduct(r));
      } catch (err) {
    ...
    if (this.supabaseClient) {
      try {
        ...
        if (!error && data) return data.map((r: any) => this.rowToProduct(r));
      } catch (err) {
